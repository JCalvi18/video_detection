...
parser.add_argument('--threshold-face', type=float, default=0.6, help='Threshold for face distances')
...
class VideoDetector(object):
    def __init__(self):
        ...
        self.location_model = 'hog' if args.gpu < 0 else 'cnn'

    @staticmethod
    def prepare_faces(dataset_name='dataset.pkl'):
        ...
        for name in face_names:
            ...
            encodings = [fr.face_encodings(img, num_jitters=5) for img in images]
            features = [en[0] for en in encodings if en]
    ...
    def draw_names(self, frame):
        ...
        for person, c in zip(self.persons, colors):
            ...
            if person.name is None:
                cv2.rectangle(frame, (int(b[1]), int(b[0])), (int(b[3]), int(b[2])), colors[-1], 2)
            else:
                cv2.rectangle(frame, (int(b[1]), int(b[0])), (int(b[3]), int(b[2])), c, 2)
                cv2.putText(frame, person.name, (int(b[1]), int(b[0])), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3,
                            cv2.LINE_AA)
        return frame

    def name_face(self, face_image, box):
        # Name the face of a person based on the dataset
        face_feature = fr.face_encodings(face_image, known_face_locations=[box], num_jitters=5)

        # Calculate the similarity between the known features and the current face feature
        sim = fr.face_distance(self.dataset, face_feature)
        scores = {}
        for known_id, index in self.names.items():
            scores[known_id] = min(sim[index])

        if min(scores.values()) < args.threshold_face:
    ...
    def name_person(self, frame, points, box, person=None):
        ...
        ret, l, r, u, d = check_large_pose(points, box)
        if ret != 4:
            name = self.name_face(frame, box)
    ...
    def detect_faces(self, frame):
        if self.dataset is None:
            self.load_features()
        # run detector
        bbox = fr.face_locations(frame, model=self.location_model)
